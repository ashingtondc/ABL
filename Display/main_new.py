...
"""
    ...
    # the code immediately below is an alternative to the code immediately above. It specifies the number of seconds
    # needed for a driver to perceive & respond to an approaching vehicle, complete the passing maneuver, and return
    # to the center lane
    ext_iaxn_time = 12
    ...
    def two_mvs_interacting_at_a_distance(self, ru2):
        ...
        """
        # Check to ensure that both road users are Motor Vehicles. Only MVs can interact at a distance
        if (self._type != 'Motor Vehicle') or (ru2._type != 'Motor Vehicle'):
            return False

        # Check to ensure that the 2 MVs are traveling in opposite directions
        ...
        # the following is old code which implemented the interacting at a distance function - no longer used because
        # it didn't include the time and distance after the passing maneuver had occurred
        # this can be removed after the new code is running correctly
        # calculate length of extended interaction areas depending on speed of the motor vehicles
        # default = 4 seconds. this is taken from the amount of time allocated to perform a lane change maneuver
        # in the Greenbook's Decision Sight Distance for a rural road as documented on p. 17 of
        # "Decision Sight Distance, Discussion Paper #2" by Dr. Robert Layton, prepared for ODOT by OSU, March 2012.
...
# this code needs to be modified in order to define an extended interaction as including the time/length after the 2
# MVs have passed one another.
...
if __name__ == '__main__':
    ...
    while clk.still_running():
        ...
        iaxns_mv_only = 0
        for ri in Interaction.recorded:
            if Interaction.contains_mv(ri):
                if Interaction.contains_vru(ri):
                    iaxns_w_vrus += 1
                else:
                    iaxns_mv_only += 1
        print('Total Number of interactions = ', iaxns_w_vrus + iaxns_mv_only, '. Number with VRUS = ', iaxns_w_vrus,
              '. Number of MVxMV only = ', iaxns_mv_only, '.')
        print('Number of interactions per hour, all types     = ',
              (iaxns_mv_only + iaxns_w_vrus) / args.time)
        print('Number of interactions per hour including VRUs = ', iaxns_w_vrus / args.time)
        print('Number of interactions per hour of MVxMV only  = ', iaxns_mv_only / args.time)
