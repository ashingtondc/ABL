...
"""
    ...
    # calculate length of extended interaction areas depending on speed of the motor vehicles
    # default = 4 seconds. this is taken from the amount of time allocated to perform a lane change maneuver
    # in the Greenbook's Decision Sight Distance for a rural road as documented on p. 17 of
    # "Decision Sight Distance, Discussion Paper #2" by Dr. Robert Layton, prepared for ODOT by OSU, March 2012.
    # Since this distance for both drivers can overlap somewhat, we use 3 for each MV
    time_to_change_lanes = 3
    ...
    def move(self):
        ...
        follow_distance = 1.5*self._speed
    ...
    def output_all_created_road_users():
        """This function outputs information on the road users created during this simulation to stdout
        """
        print()
    ...
    def interacting_at_a_distance(self, ru2):
        """Return True if the 2 oncoming motor vehicles, which have not yet come alongside or passed each other,
            are within decision sight distance of each other, else False
        """
        # calculate length of extended interaction areas depending on speed of the motor vehicles
        # default = 4 seconds. this is taken from the amount of time allocated to perform a lane change maneuver
        # in the Greenbook's Decision Sight Distance for a rural road as documented on p. 17 of
        # "Decision Sight Distance, Discussion Paper #2" by Dr. Robert Layton, prepared for ODOT by OSU, March 2012.
        # Since this distance for both drivers can overlap somewhat, we use 3 for each MV
        self_ext_area = self._speed * RoadUser.time_to_change_lanes
        ru2_ext_area = ru2._speed * RoadUser.time_to_change_lanes

        # calculate location of front of the extended interaction areas and check for overlap
        if self._direction == 'WestBound':
            self_lead_edge = self._front - self_ext_area
            ru2_lead_edge  = ru2._front + ru2_ext_area
            if self_lead_edge > ru2_lead_edge:
                return False
        else:                           # self is eastbound
            self_lead_edge = self._front + self_ext_area
            ru2_lead_edge  = ru2._front - ru2_ext_area
            if self_lead_edge < ru2_lead_edge:
                return False
        # print('2 MVs in an extended interaction. #', self._ID, ',', self._direction, 'at', self._location, '& #', ru2._ID, ',', ru2._direction, 'at', ru2._location)
        return True

    def ru_within_range(self, west_side_of_range, east_side_of_range):
        """This function returns True if any portion of the road user lies within the passed range, else False
    ...
    def create_shorthand(parties):
        """ This fcn returns a textual shorthand for the parties involved with an interaction, which is passed to it.
            ...
            e.g. there is no way to tell if 'B' in position 1 indicates one WB bike or more than one.
        """
        shorthand = '      '

        for ru in parties:                                                             # position #
            if   ru.direction() == 'WestBound' and ru.type() == 'Pedestrian':              # 0
                shorthand = 'P' + shorthand[1:]
            elif ru.direction() == 'WestBound' and ru.type() == 'Bicycle':                 # 1
                shorthand = shorthand[0] + 'B' + shorthand[2:]
            elif ru.direction() == 'WestBound' and ru.type() == 'Motor Vehicle':           # 2
                shorthand = shorthand[0:2] + 'M' + shorthand[3:]
            elif ru.direction() == 'EastBound' and ru.type() == 'Motor Vehicle':           # 3
                shorthand = shorthand[0:3] + 'M' + shorthand[4:]
            elif ru.direction() == 'EastBound' and ru.type() == 'Bicycle':                 # 4
                shorthand = shorthand[0:4] + 'B' + shorthand[5:]
            elif ru.direction() == 'EastBound' and ru.type() == 'Pedestrian':              # 5
                shorthand = shorthand[0:5] + 'P'
            else:
                # unknown direction/type of road user - ignore for now
                pass
    ...
    def set_extended_interaction_time(seconds):
        ...
        """
        RoadUser.time_to_change_lanes = seconds
...
class Interaction:
    ...
    Once they finish, they are moved from the Active set to the Recorded set.

    Attributes
        InteractionID: Unique ID
        RoadUsers: list of Road Users which are interacting with each other
    ...
    def __init__(self, road_users, start_time, end_time, extended):
    ...
    def exists(self, ru2):
        ...
        # Case #1
        # Are these 2 road users overlapping, i.e. are they alongside each other?
        if not((ru2.west_end() > self.east_end()) or (ru2.east_end() < self.west_end())):
            return 1

        # Case #2
        # if 2 oncoming vehicles have not yet passed or come alongside each other and
        # their extended interaction areas overlap, then they are interacting at a distance
        if RoadUser.oncoming_MVs(self, ru2) and RoadUser.interacting_at_a_distance(self, ru2):
            return 2
    ...
    def identify_current_interactions():
        ...
        for ru1, ru2 in combinations(RoadUser.OnRoad, 2):
            # print('testing 2 RUs for an interaction', ru1._ID, ru2._ID)
            interaction_type = Interaction.exists(ru1, ru2)
            if interaction_type == 1:
                Interaction.current.append(Interaction([ru1, ru2], clk.real_time(), -1, False))
            elif interaction_type == 2:
                Interaction.current.append(Interaction([ru1, ru2], clk.real_time(), -1, True))
        ...
        #     print('current interactions identified at time', clk.real_time(), 'are:')
        #     for i in Interaction.current:
        #         print('     ID', i._ID, ', road users=', [ru._ID for ru in i._parties], ', start time=', i._start_time)
        # else:
        #     print('No current interactions at time', clk.real_time(), 'seconds')

        # aggregate all interactions which are components of a multi-party interaction
    ...
    def update_interactions():
        ...
        # 1. if a corresponding interaction exists in current, this ongoing interaction is removed from current
        # 2. if no corresponding interaction exists in current, the interaction has finished and is moved into recorded.
        # at the end of this code, the remaining current interactions are new and need to be moved into active
        # need a more pythonic approach to this logic
        for i in range(len(Interaction.active) - 1, -1, -1):
            for j in range(len(Interaction.current) - 1, -1, -1):
                if set(Interaction.current[j]._parties) == set(Interaction.active[i]._parties):
                    # print('found ongoing i-action, current index', j, 'ID', current[j]._ID, [ru._ID for ru in current[j]._parties], 'active index', i, 'ID', active[i]._ID, [ru._ID for ru in active[i]._parties])
    ...
    def aggregate_current_interactions():
        ...
        if len(Interaction.current) > 1:                     # need enough current interactions to aggregate
            ...
            while aggregation_not_done:
                ...
                # if 2 interactions share a road user, they are combined into one interaction
                aggregation_not_done = False
                for ci1, ci2 in combinations(Interaction.current, 2):
                    if Interaction.share_road_users(ci1, ci2):
                        ci3 = Interaction.combine_interactions(ci1, ci2)
                        # print('2 interactions share road users - combine', '#', ci1._ID,
                        #       [ru._ID for ru in ci1._parties], 'and #', ci2._ID, [ru._ID for ru in ci2._parties],
                        #       'to create #', ci3._ID, [ru._ID for ru in ci3._parties])
    ...
    def combine_interactions(self, i2):
        ...
        i3 = Interaction(road_users, start_time, end_time, extended)
    ...
    def output_all_created_interactions():
        ...
        for ri in Interaction.recorded:
            print(ri)
            # print('ID:', ri._ID, 'code:', ri._shorthand, 'road users:', [ru._ID for ru in ri._parties],
            #       ', start time:', ri._start_time, ', end time:', ri._end_time,
            #       ', start location:', ri._start_location, ', end location:', ri._end_location,
            #       ', extended:', ri._extended)
...
def initialize_everything():
    ...
    if args.test:
        ...
        # determine number of road users per hour for each type and each end of the road
...
if __name__ == '__main__':
    ...
    parser.add_argument("-v",  "--mvhr",      type=int,   default=100,      help="specify # of motor vehicles per hour, "
                        "range 0-3600")
    # do we want to add ranges to ped and bike volumes as well?

    parser.add_argument("-cp", "--cycsplit", type=int, default=50, help="specify % of cyclist volume traveling east,"
            "the remaining volume will travel west. range 0-100")
    parser.add_argument("-pp", "--pedsplit", type=int, default=50, help="specify % of pedestrian volume traveling east,"
            "the remaining volume will travel west. range 0-100")
    ...
    parser.add_argument("-cd", "--cycspeeddistr",  type=int, default=1,    help="specify cyclist speed distribution"
            "(0=all cyclists travel at cycspeed, 1=normal distribution with 85th percentile at cycspeed value)")
    parser.add_argument("-pd", "--pedspeeddistr",  type=int, default=1,    help="specify pedestrian speed distribution"
            "(0=all pedestrians travel at pedspeed, 1=normal distribution with 85th percentile at pedspeed value)")
    parser.add_argument("-vd", "--mvspeeddistr",   type=int, default=1,    help="specify vehicle speed distribution"
            "(0=all vehicles travel at mvspeed, 1=normal distribution with 85th percentile at mvspeed value)")
    ...
    # sys.stdout = open('C:\\Users\Michael\Downloads\output file.txt', 'w')
    ...
    if not args.test:
        print('Simulation began at', str(datetime.datetime.now()))
        print()

    print('Command line or default parameter values')
   ...
   # SIMULATION LOOP STARTS HERE  *****************************************************************************
    ...
    # remove redirection of stdout to file which is done for testing purposes only
